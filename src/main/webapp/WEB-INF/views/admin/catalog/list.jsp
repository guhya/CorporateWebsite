			...
			<div class="col-lg-12">
				<section class="panel">
					...
					<div class="panel-body">
						<form name="searchForm" class="form-inline" role="form" method="get" action="/admin/catalog/list">				
							...
							<button type="button" onclick="javascript:top.location='/admin/catalog/write?<c:out value="${parameter.parameterLink}" />'" class="btn btn-lg btn-success pull-right">
							...
							<div class="form-group">
								<div class="input-group" style="max-width: 320px;">
									...
									<select name="tmpSortColumn" id="tmpSortColumn" class="form-control" aria-describedby="sb">
										...
										<option value="name" <c:if test="${parameter.sortColumn == 'name'}">selected</c:if>>Catalog Name</option>
			...
			<div class="col-lg-12">
				<section class="panel table-responsive">
					<table class="table table-striped table-advance table-hover">
						<tbody>
							<tr>
								<th class="col-md-3"><i class="fa fa-info-circle">&nbsp;</i> Catalog Name</th>
								<th class="col-md-4"><i class="fa fa-comment">&nbsp;</i> Catalog Description</th>
							...
							<c:if test="${not empty catalog}">
								<c:forEach var="item" items="${catalog}" varStatus="status">
									<tr>
										<td onclick="location.href='/admin/catalog/detail?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />'">
											<a href="/admin/catalog/detail?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />">
												<p><c:out value="${item.name}"/></p>
											</a>
										</td>
										<td class="">
											<c:set var="desc" value="${ew:stripTags(item.description)}" />
										...
										<td class="">
											<div class="btn-group pull-right">
												<a href="/admin/catalog/detail?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />" class="btn btn-info"><i class="fa fa-search"></i> Detail</a>
												<a href="/admin/catalog/edit?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />" class="btn btn-success"><i class="fa fa-edit"></i> Edit</a>
											</div>
										</td>
									</tr>
								</c:forEach>
