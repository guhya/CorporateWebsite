...
<tiles:insertDefinition name="_baseAdmin">
    ...
    <tiles:putAttribute name="body">
		...
		<div class="row">
			<div class="col-lg-12">
				<h3 class="page-header">
					<i class="fa fa-graduation-cap"></i> Career List
				</h3>
				<ol class="breadcrumb">
					<li><i class="fa fa-home"></i><a href="/admin/dashboard">Home</a></li>
					<li><i class="fa fa-list"></i>Career List</li>
				</ol>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-lg-12">
				<section class="panel">
					<header class="panel-heading"> Search Career </header>
					<div class="panel-body">
						<form name="searchForm" class="form-inline" role="form" method="get" action="/admin/career/list">				
							...
							<button type="button" onclick="javascript:top.location='/admin/career/write?<c:out value="${parameter.parameterLink}" />'" class="btn btn-lg btn-success pull-right">
							...
							<div class="form-group">
								<div class="input-group" style="max-width: 320px;">
									...
									<select name="tmpSortColumn" id="tmpSortColumn" class="form-control" aria-describedby="sb">
										...
										<option value="title" <c:if test="${parameter.sortColumn == 'title'}">selected</c:if>>Career Title</option>
			...
			<div class="col-lg-12">
				<section class="panel table-responsive">
					<table class="table table-striped table-advance table-hover">
						<tbody>
							<tr>
								<th class="col-md-2"><i class="fa fa-info-circle">&nbsp;</i> Career Title</th>
								<th class="col-md-4"><i class="fa fa-comment">&nbsp;</i> Career Content</th>
							...
							<c:if test="${not empty career}">
								<c:forEach var="item" items="${career}" varStatus="status">
									<tr>
										<td onclick="location.href='/admin/career/detail?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />'">
											<a href="/admin/career/detail?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />">
										...
										<td class="">
											<div class="btn-group pull-right">
												<a href="/admin/career/detail?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />" class="btn btn-info"><i class="fa fa-search"></i> Detail</a>
												<a href="/admin/career/edit?seq=<c:out value="${item.seq}" />&<c:out value="${parameter.parameterLink}" />" class="btn btn-success"><i class="fa fa-edit"></i> Edit</a>
											</div>
										</td>
									</tr>
								</c:forEach>
