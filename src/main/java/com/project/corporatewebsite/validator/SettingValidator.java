...
public class SettingValidator implements Validator {
	...
	public void validateWithFile(Object target, List<Map<String, MultipartFile>> fileList, Errors errors, String mode) {
		...
		String key = "";
		boolean passThumbnail = false, passMainImage = false;
		...
		if(Constants.WRITE.equals(mode)){
			if (!passThumbnail) errors.reject(Constants.THUMBNAIL_IMAGE, "Image is required");
			if (!passMainImage) errors.reject(Constants.MAIN_IMAGE, "Image is required");
		}
